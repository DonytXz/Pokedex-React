      ...
      <div className="flex flex-wrap gap-1 mt-1.5">
        {types && types.length
          ? types.map((type, index) => (
              <Types key={index} type={type.type || type} />
            ))
          : null}
      </div>
...
const Modal = (props) => {
  ...
  const handleTouchEnd = (e) => {
    ...
    const start = { x: touchStartXRef.current, y: touchStartYRef.current };
    const end = { x: e.changedTouches[0].clientX, y: e.changedTouches[0].clientY };
    const direction = getSwipeDirection(start, end);

    if (direction === "next" && hasNext && typeof onNextPokemon === "function") {
      onNextPokemon();
    } else if (direction === "prev" && hasPrev && typeof onPrevPokemon === "function") {
      onPrevPokemon();
    }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key === "ArrowRight" && hasNext && typeof onNextPokemon === "function") {
        e.preventDefault();
        onNextPokemon();
        return;
      }
